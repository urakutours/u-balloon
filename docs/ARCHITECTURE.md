...
本ドキュメントは **u-balloon 単独インスタンスの現行実装**を記述している。u-balloon は 2026-04 時点で、単独 EC ショップから「**汎用 EC プラットフォームの第 1 インスタンス**」へ位置付け直されつつある。

### 方針の要約

- アーキタイプ: **Y 型（マルチインスタンス、pnpm monorepo）** を採用
- 将来の X 型化（マルチテナント SaaS）余地は設計上残す
- u-balloon の本番体験は壊さず、Phase 単位で段階移行する
- バルーン特化要素（約 230 件のコピー・5 件の DB レベル特化）は削除せず、**業態テンプレートプラグイン `industry-balloon` として分離**する

### 現行実装との関係

本 ARCHITECTURE.md の § 1〜13 は **引き続き正**。汎用化リファクタは別 branch で進行し、Phase 5（モノレポ化）まで u-balloon 本体のデプロイ構成は変わらない。Phase 6（コア抽出）以降、`packages/core` への import 経由で動作する構成に段階的に移行する。

### 詳細は別ドキュメント

全体方針・アーキテクチャ選定理由・Phase 0〜9 ロードマップ・オープンクエスチョンは以下に集約:

→ [`../../../ec-platform/PLATFORM_VISION.md`](../../../ec-platform/PLATFORM_VISION.md)

実装変更を伴う作業に着手する前に、上記 PLATFORM_VISION.md を参照して当該作業がどの Phase に属するかを確認すること。
