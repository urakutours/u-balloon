# プロンプト 2: MakeShop ステージングリハーサル実施 (確定版 v3.1)

> **作成日**: 2026-04-24 (v3.1: B6 path 修正 + 事前準備履歴反映)
> **想定実施日**: 2026-04-24 〜 2026-04-25
> **本番切替予定**: 2026-04-30 05:00-07:00 JST
> **関連**: `u-balloon/docs/migration/rehearsal-checklist.md`
>
> 本プロンプトは Claude Code が可能な限り自動実行する設計。Daisuke 手動は「事前準備」「各 Step 境界応答」「/account UI 目視 1 件」「最終判断」のみ。
>
> **v3 → v3.1 変更点**:
> - B6: Step 0-2 の path 誤記修正 (`src/scripts/...` → `scripts/...`)
> - importMap.js 関連の build script を `node scripts/patch-payload-paginator.mjs && next build` に戻し(v3 で提案した `payload generate:importmap` はローカル動作不良、untrack を revert することで解決済み)
> - Step 0-2 の検証行に importMap.js が tracked であることの確認を追加
...
# ===== リハーサル固有 =====
REHEARSAL_DATE=2026-04-24
REHEARSAL_CSV_PATH=tmp/rehearsal/makeshop-export.tsv
```
...
for v in NEON_API_KEY NEON_PROJECT_ID NEON_PRODUCTION_BRANCH_ID VERCEL_TOKEN VERCEL_PROJECT_ID ADMIN_EMAIL ADMIN_PASSWORD RESEND_API_KEY_REHEARSAL REHEARSAL_DATE REHEARSAL_CSV_PATH; do
...
SELECT legacy_id, total_orders, total_spent
